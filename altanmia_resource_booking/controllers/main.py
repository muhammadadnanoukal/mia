...
class Booking(http.Controller):
    ...
    def calendar_booking_update_available_slots(self, book_profile_id, resource_id=None, timezone=None, **kwargs):
        ...
        if resource_id:
            resource = request.env['hr.employee'].sudo().browse(int(resource_id)) \
                if booking_proifle.resource_type == 'person' else request.env['tanmia.booking.place'].sudo().browse(
                int(resource_id))
    ...
    def check_resource_availability(self, book, resource_id, date_start, date_end):
        resource = None
        redirect = None
        if book.resource_type == 'person':
            resource = request.env['hr.employee'].sudo().browse(int(resource_id)).exists()
            if resource not in book.sudo().employee_ids:
                raise NotFound()
            ...
            if resource.user_id and resource.user_id.partner_id:
                if not resource.user_id.partner_id.calendar_verify_availability(date_start, date_end):
                    redirect = request.redirect('/booking/%s/profile?state=failed-employee' % slug(book))

        elif book.resource_type == 'place':
            resource = request.env['tanmia.booking.place'].sudo().browse(int(resource_id)).exists()
            if resource not in book.sudo().places_ids:
                raise  NotFound()
    ...
    def booking_info(self, book, dates, **post):
        ...
        for d in dates:
            ...
            booking_order.add_date(date=event)
    ...
    def checkout_redirection(self, order):
        ...
        for line in order.order_line:
            ...
            timezone = request.session['timezone'] or book.appointment_tz
            tz_session = pytz.timezone(timezone)
            resource = event.user_id if book.resource_type == 'person' else event.place_id
