...
class BookProfile(models.Model):
    ...
    places_ids = fields.Many2many('tanmia.booking.place', string='Places')
    ...
    def _slot_availability_is_employee_available(self, slot, staff_employee, availability_values):
        ...
        place_reservation = availability_values.get('place_reservations') or {}
        resource_event = partner_to_events if self.resource_type =='person' else place_reservation

        reserved = 0
        if is_available and resource_event.get(resouce):
            for day_dt in rrule(freq=DAILY,
                                      dtstart=slot_start_dt_utc,
                                      until=slot_end_dt_utc,
                                      interval=1):
                day_events = resource_event[resouce].get(day_dt.date()) or []
                for event in day_events:
                    if event.allday or (event.start < slot_end_dt_utc and event.stop > slot_start_dt_utc):
                        reserved += 1

            if self.resource_type == 'person' and reserved > 0:
                is_available = False
            elif self.resource_type == 'place' and reserved >= resouce.capacity:
                is_available = False
    ...
    def get_available_days(self, *args, **kwargs):
        ...
        employee = self.env['hr.employee'].browse(int(employee_id)) if appointment_type.resource_type == 'person' else self.env['tanmia.booking.place'].browse(int(employee_id))
