...
class SaleOrder(models.Model):
    ...
    def add_date(self, date=None, **kwargs):
        """ Add or set product quantity, add_qty can be negative """
        product_id = date.book_profile_id.as_product
        add_qty = 1
        set_qty = 0
        self.ensure_one()
    ...
    def onchange_partner_id(self):
        ...
        for line in self.order_line:
            line.booking_event.partner_id = self.partner_id

    def make_reservation(self):
        for order in self:
            for line in order.order_line:
                line.booking_event.write({'active':True})

class SaleOrderLine(models.Model):
    _inherit = "sale.order.line"

    booking_event = fields.Many2one('calendar.event', string='Booking Event')
