...
class TanmyaProductemplateExt(models.Model):
    ...
    recipe_status = fields.Selection([('private','Private Recipe'),
                                      ...
                                      string='Recipe Status', default='private', readonly=True)
    ...
    def get_user_info(self, user_id:int):
        if user_id:
            ...
            if user:
                ...
                if partner:
                    return [partner.name, partner.email, partner.image_128, partner.phone]

    @api.model
    def get_count_user_recipes(self, user_id:int):
        if user_id:
            return int(len(self.env['product.product'].search([('owner_id','=',user_id)])))
        return 0
