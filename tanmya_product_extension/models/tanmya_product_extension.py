...
class TanmyaProductemplateExt(models.Model):
    ...
    def get_user_info(self, user_id:int):
        if user_id:
            user = self.env['res.users'].serach([('id','=',user_id)])
            if user:
                partner = self.env['res.partner'].search([('id','=',user.partner_id)])
                if partner:
                    return [partner.name, partner.email, partner.__last_update, partner.image_128]
