...
Select fin.*,emp.studio_employee_number from
(
select S.dt,S.dt+interval '1 hours'*(C.hour_from )+interval '1 hours'*{diff_h}  as point0,
case when COALESCE(C.duration,0)=0 then C.hour_to-C.hour_from else C.duration end as dur,
...
(select G.dt,extract(isodow from G.dt) - 1 as dayofweek,  ((cast(to_char( dt+interval '0 day' ,'J') as integer)-1721424)/7) %2 as iweek
...
SELECT dt,dur,point0 as point, 0 as check,
    id,studio_employee_number,tol_in_early as delay_early,tol_in_late as delay_late
	FROM temp_stuff
union
SELECT dt,dur,point1 as point, 1 as check,
    id,studio_employee_number,tol_out_early as delay_early,tol_out_over as delay_late
	FROM temp_stuff
